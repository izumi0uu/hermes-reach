...
Connector 的身份、在线授权、固定 TLS、原终端解锁、VPS 配对与本地可用性快照已经有基础实现。**但协议还没有携带受保护请求和规范化结果，VPS 客户端也没有接入普通 `reach_*` 请求，因此这条远程安全链路目前不能视为生产可用。**
...
| 规划中 | Twitter/X、Reddit、小红书、Facebook、Instagram、LinkedIn、雪球、小宇宙 | 等待 Connector 和凭据隔离 |
...
普通请求目前由 Hermes Reach 自己的运行时和本地适配器执行。Agent-Reach 当前只提供固定版本的平台目录、执行后端信息和显式检查。
...
flowchart TD
    ...
    Grant --> Binding["精确 Agent-Reach 接口绑定"]
    Secrets["Bitwarden 与本地会话"] --> Binding
    Binding --> Backend["Agent-Reach 平台后端"]
...
| 当前 | 安全配对与客户端基础 | ConnectorClient、固定身份与授权、本地可用性快照、签名请求和回执 |
| 下一步 | 隔离凭据并冻结执行协议 | Bitwarden 凭据接口、受保护请求与规范化结果 envelope |
